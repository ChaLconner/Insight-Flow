...
@pytest.mark.asyncio
async def test_notify_task_assigned_success(trigger_service, mock_db_session, mock_rate_limiter, users):
    assigner, assignee = users
    task_id = uuid.uuid4()
    project_id = uuid.uuid4()
    
    # Mock user preferences (empty returns defaults -> True)
    with patch.object(trigger_service, "_get_user_preferences", return_value={"inApp": {"tasks": True}}):
...
@pytest.mark.asyncio
async def test_notify_task_assigned_rate_limited(trigger_service, mock_db_session, mock_rate_limiter, users):
    assigner, assignee = users
...
async def test_notify_task_assigned_disabled_pref(trigger_service, mock_db_session, users):
    ...
    with patch.object(trigger_service, "_get_user_preferences", return_value={"inApp": {"tasks": False}}):
        await trigger_service.notify_task_assigned(
            assignee=assignee,
            task_id=uuid.uuid4(),
            task_title="Test Task",
...
async def test_notify_project_member_added(trigger_service, mock_db_session, users):
    ...
    with patch.object(trigger_service, "_get_user_preferences", return_value={"inApp": {"projects": True}}):
...
async def test_grouping_existing_notification(trigger_service, mock_db_session, users):
    ...
    existing_notif = Notification(id=uuid.uuid4(), type="task_updated", data={"count": 1, "items": []})
    
    with patch.object(trigger_service, "_get_user_preferences", return_value={"inApp": {"tasks": True}}), \
         patch.object(trigger_service, "_find_existing_group_notification", return_value=existing_notif):
