...
@pytest.mark.asyncio
async def test_create_project_success(async_project_service, test_user, project_data, async_session):
    # Act
...
@pytest.mark.asyncio
async def test_create_project_limit_check(async_project_service, test_user, project_data, async_session):
    # Setup - mock subscription limit
    # Default is FREE which has 2 projects. 
    # Let's create 2 projects first
    await async_project_service.create_project(
        ProjectCreate(name="P1"), test_user.id
    )
    await async_project_service.create_project(
        ProjectCreate(name="P2"), test_user.id
    )
    
    # Act & Assert - Try to create 3rd
    with pytest.raises(ValueError, match="Project limit reached"):
        await async_project_service.create_project(
            ProjectCreate(name="P3"), test_user.id
        )
...
@pytest.mark.asyncio
async def test_update_project_not_owner(async_project_service, test_user, project_data, async_session):
    project = await async_project_service.create_project(project_data, test_user.id)
...
async def test_add_project_member(async_project_service, test_user, project_data, async_session):
    ...
    member = await async_project_service.add_project_member(
        project.id, 
        ProjectMemberCreate(user_id=str(other_user.id), role="admin"),
        test_user.id
    )
...
async def test_remove_project_member(async_project_service, test_user, project_data, async_session):
    ...
    await async_project_service.add_project_member(
        project.id,
        ProjectMemberCreate(user_id=str(other_user.id), role="member"),
        test_user.id
    )
...
@pytest.mark.asyncio
async def test_add_project_members_bulk(async_project_service, test_user, project_data, async_session):
    project = await async_project_service.create_project(project_data, test_user.id)
...
async def test_check_member_limit(async_project_service, test_user, async_session):
    ...
    await async_session.commit()
    await async_session.refresh(u1); await async_session.refresh(u2); await async_session.refresh(u3)
    
    # Add 2 members -> Total 3. OK.
    await async_project_service.add_project_member(p.id, ProjectMemberCreate(user_id=str(u1.id), role="member"), test_user.id)
    await async_project_service.add_project_member(p.id, ProjectMemberCreate(user_id=str(u2.id), role="member"), test_user.id)
    
    # Add 3rd member -> Total 4. Should Fail.
    with pytest.raises(ValueError, match="Team member limit reached"):
        await async_project_service.add_project_member(p.id, ProjectMemberCreate(user_id=str(u3.id), role="member"), test_user.id)
