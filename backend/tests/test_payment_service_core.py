...
from models.payment import PaymentHistory, PaymentMethod, PaymentStatus, Subscription, SubscriptionPlan, SubscriptionStatus
...
async def test_get_or_create_stripe_customer_cached_on_user_obj(payment_service, mock_db_session):
    ...
    with patch("stripe.Customer.retrieve") as mock_retrieve:
        ...
        cid = await payment_service.get_or_create_stripe_customer(mock_db_session, user_id, "test@test.com", user=user)
...
async def test_get_or_create_stripe_customer_create_new(payment_service, mock_db_session):
    ...
    with patch("stripe.Customer.list", return_value=result_list) as mock_list, \
...
async def test_attach_payment_method(payment_service, mock_db_session):
    user_id = uuid4()
    data = MagicMock(payment_method_id="pm_123", set_as_default=True, billing_address=None, billing_name="Test")
    customer_id = "cus_123"
    ...
    with patch("stripe.PaymentMethod.retrieve", return_value=pm_stripe), \
         patch("stripe.PaymentMethod.attach") as mock_attach, \
         patch("stripe.Customer.modify") as mock_modify:
        
        result = await payment_service.attach_payment_method(mock_db_session, user_id, data, customer_id)
...
async def test_list_payment_history(payment_service, mock_db_session):
    ...
    # Mock Items
    items_res = MagicMock()
    items_res.scalars.return_value.all.return_value = [PaymentHistory(id=uuid4()), PaymentHistory(id=uuid4())]
...
async def test_delete_payment_method(payment_service, mock_db_session):
    ...
    pm = PaymentMethod(id=pm_id, stripe_payment_method_id="pm_stripe_123", is_default=False, user_id=user_id)
...
async def test_delete_payment_method_auto_promote(payment_service, mock_db_session):
    ...
    pm = PaymentMethod(id=pm_id, stripe_payment_method_id="pm_stripe_123", is_default=True, user_id=user_id)
    other_pm = PaymentMethod(id=uuid4(), stripe_payment_method_id="pm_stripe_456", is_default=False, user_id=user_id)
    ...
    with patch("stripe.PaymentMethod.detach") as mock_detach:
        ...
        assert result is True
        payment_service.set_default_payment_method.assert_called_once_with(mock_db_session, other_pm.id, user_id)
