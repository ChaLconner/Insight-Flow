...
from services.async_analytics_service import AsyncAnalyticsService, ANALYTICS_CACHE_TTL
...
async def test_get_analytics_overview_success(analytics_service, user_id, project_ids):
    # Mock cache miss
    with patch("services.cache_service.cache_service.get", return_value=None), \
         patch.object(analytics_service, "_get_overview_metrics", return_value={"totalProjects": 2}) as mock_overview, \
         patch.object(analytics_service, "_get_project_stats", return_value=[{"name": "P1"}]) as mock_proj_stats, \
         patch.object(analytics_service, "_get_team_stats", return_value=[{"name": "U1"}]) as mock_team_stats, \
         patch.object(analytics_service, "_get_trends", return_value=[{"metric": "Tasks"}]) as mock_trends, \
         patch.object(analytics_service, "_get_distributions", return_value={"status": []}) as mock_dist, \
         patch.object(analytics_service, "_get_daily_trends", return_value=[{"date": "Jan 1", "created": 0, "completed": 0}]) as mock_daily:
...
async def test_get_project_contributions(analytics_service):
    ...
    assert result["contributors"][0]["percentage"] == 100 # 20/20 tasks of total found (if only 1 user) or 0? 
