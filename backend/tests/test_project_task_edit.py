...
def test_unauthorized_task_access(unauthenticated_client: TestClient, db_session: Session, test_user: User):
    ...
    task = Task(title="Secret", status=TaskStatus.TODO, project_id=project.id, created_by=test_user.id)
    db_session.add(task)
    db_session.commit()
