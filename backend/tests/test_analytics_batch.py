...
async def test_get_batch_recent_activity():
    ...
    try:
        with patch('services.async_task_history_service.AsyncTaskHistoryService') as MockTaskService, \
            ...
            mock_task_service.get_recent_activities_for_projects = AsyncMock(return_value=[activity])
            ...
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as ac:
                response = await ac.post("/api/v1/analytics/activity/batch", json={
                    "project_ids": [str(activity.project_id)],
                    "limit": 5
                })
...
def test_analytics_batch_endpoint_requires_auth():
    ...
    response = client.post("/api/v1/analytics/activity/batch", json={
