...
def test_search_users_list(client, mock_user_service):
    ...
    assert response.status_code == 200
    mock_user_service.search_users.assert_called_with("test", skip=0, limit=20, role="admin", is_active=True)
...
def test_upload_avatar_local_success(client, mock_user_service, mock_current_user):
    ...
    with patch("routers.users.is_cloudinary_configured", return_value=False), \
...
def test_upload_avatar_cloudinary_success(client, mock_user_service, mock_current_user):
    ...
    with patch("routers.users.is_cloudinary_configured", return_value=True), \
