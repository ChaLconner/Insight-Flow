...
from services.async_user_service import AsyncUserService, AuthStatus
from models.user import User
from models.auth_audit import AuthAudit
from models.user_settings import UserSettings
from schemas.user import UserCreate, UserLogin, UserUpdate, UserSettingsUpdate, UserInvite
...
@pytest.fixture
def mock_auth_utils():
    with patch("services.async_user_service.get_password_hash") as mock_hash, \
...
@pytest.mark.asyncio
async def test_create_user_success(user_service, mock_db_session, mock_email_service, mock_auth_utils, mock_validators):
...
@pytest.mark.asyncio
async def test_change_password_success(user_service, mock_db_session, mock_auth_utils, mock_validators):
    uid = uuid.uuid4()
