...
def seed_analytics_data():
    ...
    try:
        ...
        team_members = []
        member_names = ["Sarah Chen", "Mike Ross", "Alex Morgan", "John Doe", "Emily Blunt", "David Kim"]
        ...
        project_names = ["Q4 Financial Report", "Customer Portal V2", "Internal Tools Migration", "AI Integration Alpha"]
        projects = []
        
        for p_name in project_names:
            project = db.query(Project).filter(Project.name == p_name).first()
            if not project:
                project = Project(
                    id=uuid.uuid4(),
                    name=p_name,
