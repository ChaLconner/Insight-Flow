...
class NotificationRateLimiter:
    ...
    def _cleanup_old_entries(self, user_id: str):
        ...
        self._notifications[user_id] = [
            (ts, t) for ts, t in self._notifications[user_id]
            if ts > cutoff
        ]
    ...
    def can_send(self, user_id: str, notification_type: str) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            if hourly_count >= self.max_per_hour:
                logger.warning(f"Rate limit: User {user_key} exceeded hourly limit ({hourly_count}/{self.max_per_hour})")
            ...
            if daily_count >= self.max_per_day:
                logger.warning(f"Rate limit: User {user_key} exceeded daily limit ({daily_count}/{self.max_per_day})")
                return False, f"Daily limit reached ({self.max_per_day}/day)"
            
            # Count same type notifications in last hour
            same_type_count = sum(1 for ts, t in entries if ts > hour_ago and t == notification_type)
            if same_type_count >= self.max_same_type_per_hour:
                logger.warning(f"Rate limit: User {user_key} exceeded type limit for {notification_type}")
...
_rate_limiter: NotificationRateLimiter = None
