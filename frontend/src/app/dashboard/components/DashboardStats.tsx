...
const DashboardStats = memo(function DashboardStats({ stats }: DashboardStatsProps) {
    ...
    const statsCards = useMemo(() => {
        return STATS_CONFIG.map((config) => {
            ...
            const change = stats[config.changeKey as keyof DashboardStatsData] as string | undefined;
            const trend = stats[config.trendKey as keyof DashboardStatsData] as 'up' | 'down' | undefined;
