...
const renderDescription = (text: string) => {
    if (!text) { return <span className="text-zinc-600 italic">No description provided.</span>; }
    ...
    lines.forEach((line, index) => {
        ...
        if (isBullet || isNumber) {
            ...
            listBuffer.push(<li key={`li-${index}`} className="ml-4 pl-1">{content}</li>);
        } else {
            if (inList) {
                elements.push(listType === 'ul' ? <ul key={`ul-${index}`} className="list-disc mb-4 space-y-1">{listBuffer}</ul> : <ol key={`ol-${index}`} className="list-decimal mb-4 space-y-1">{listBuffer}</ol>);
                listBuffer = [];
                inList = false;
            }
            if (trimmed) {
                elements.push(<p key={`p-${index}`} className="mb-2 min-h-[1.5em]">{line}</p>);
            } else {
    ...
    if (inList && listBuffer.length > 0) {
        elements.push(listType === 'ul' ? <ul key="ul-last" className="list-disc mb-4 space-y-1">{listBuffer}</ul> : <ol key="ol-last" className="list-decimal mb-4 space-y-1">{listBuffer}</ol>);
    }
    return <div className="text-zinc-300 leading-relaxed text-[15px]">{elements}</div>;
};

export function TaskDetails({ task: initialTask, backLink, backLabel }: TaskDetailsProps) {
    ...
    const getStatusColor = (s: string) => {
        switch (s) {
            ...
            case TaskStatus.IN_REVIEW: return "bg-purple-500/10 text-purple-400 border-purple-500/20";
    ...
    const getPriorityColor = (p: string) => {
        switch (p) {
            case TaskPriority.URGENT: return "text-fuchsia-400 bg-fuchsia-400/10 border-fuchsia-400/20";
            case TaskPriority.HIGH: return "text-red-400 bg-red-400/10 border-red-400/20";
            case TaskPriority.MEDIUM: return "text-amber-400 bg-amber-400/10 border-amber-400/20";
            case TaskPriority.LOW: return "text-green-400 bg-green-400/10 border-green-400/20";
    ...
    const priorityOptions = Object.values(TaskPriority).map(p => ({
        value: p, label: p.charAt(0).toUpperCase() + p.slice(1), color: getPriorityColor(p)
    }));
    const statusOptions = Object.values(TaskStatus).map(s => ({
        value: s, label: s.replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase()), color: getStatusColor(s)
    }));
    const typeOptions = Object.values(TaskType).map(t => ({
        value: t, label: t.charAt(0).toUpperCase() + t.slice(1), color: "text-zinc-400"
    }));
    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <Link href={backLink} passHref>
                <Button variant="ghost" size="sm" className="p-0 h-auto hover:bg-transparent text-zinc-400 hover:text-white mb-2">
            ...
            <div className="bg-[#18181b]/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 relative">
                ...
                <div className="absolute top-6 right-6 flex items-center gap-2">
                    {isEditing ? (
                        <>
                            <Button size="sm" variant="ghost" onClick={() => setIsEditing(false)} className="text-zinc-400 hover:text-white">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48 bg-[#18181b] border-white/10 z-50">
                                <DropdownMenuItem onClick={() => setIsEditing(true)} className="text-zinc-300 hover:text-white hover:bg-white/10 cursor-pointer">
                                    <Pencil className="h-4 w-4 mr-2" />
                                    Edit Task
                                </DropdownMenuItem>
                                <DropdownMenuSeparator className="bg-white/10" />
                                <DropdownMenuItem onClick={() => setIsDeleteModalOpen(true)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 cursor-pointer">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 text-sm flex-wrap text-zinc-400">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                            {isEditing ? (
                                <>
                                    <div className="w-32">
                                        <CustomSelect value={priority} onChange={(v) => setPriority(v as TaskPriority)} options={priorityOptions} className="w-full" />
                                    </div>
                                    <div className="w-32">
                                        <CustomSelect value={type} onChange={(v) => setType(v as TaskType)} options={typeOptions} className="w-full" />
                                    </div>
                                    <div className="w-40">
                                        <CustomSelect value={status} onChange={(v) => setStatus(v as TaskStatus)} options={statusOptions} className="w-full" />
                                    </div>
                                </>
                            ) : (
                                <>
                                    {/* Priority */}
                                    <span className={`flex items-center gap-1.5 ${getPriorityColor(task.priority).split(' ')[0]}`}>
                                    ...
                                    <Badge className={`${getStatusColor(task.status)} border px-2.5 py-0.5 text-xs uppercase tracking-wider ml-2`}>
                    ...
                    <div className="space-y-6">
                        <Card className="border-0 bg-transparent shadow-none p-0">
                            <CardHeader className="px-0 pt-0">
                                <CardTitle className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Details</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-6 px-0">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {isEditing ? (
                                            <Input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="bg-black/20 border-white/10 text-white h-8 text-xs" />
                                        ) : (
