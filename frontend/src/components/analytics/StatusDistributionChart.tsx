...
const formatStatusName = (name: string): string => {
    ...
    return nameMap[key] || name.replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase());
};

const StatusDistributionChartComponent: React.FC<StatusDistributionChartProps> = ({ data = [] }) => {
                        ...
                        <PieChart>
                            ...
                            >
                                {formattedData.map((entry, index) => (
                                    <Cell
                                        key={`cell-${index}`}
                                        fill={STATUS_COLORS[entry.name.toLowerCase().replace(' ', '_')] || '#6b7280'}
                                        strokeWidth={0}
                                    />
                                ))}
                            </Pie>
