    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            return Array.isArray(data)
                ? data.map((p: any, index: number) => transformProjectData(p, user ?? undefined, index))
                : [];
        },
        enabled: options?.enabled !== undefined ? options.enabled && !!user : !!user, // Only fetch if user is logged in
...
export function useArchiveProject() {
    ...
    return useMutation({
        ...
        onMutate: async (project) => {
            ...
            queryClient.setQueryData(["projects"], (old: Project[] | undefined) => {
                return old ? old.map(p => p.id === project.id ? { ...p, status: ProjectStatus.ARCHIVED } : p) : [];
            });
