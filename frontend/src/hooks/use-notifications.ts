...
export const useNotifications = () => {
  ...
  const unreadNotifications =
    notificationSelectors.getUnreadNotifications(store);
  const readNotifications = notificationSelectors.getReadNotifications(store);
  const filteredNotifications =
    notificationSelectors.getFilteredNotifications(store);
  const notificationsByPriority =
    notificationSelectors.getNotificationsByPriority(store);
  const notificationsByType =
    notificationSelectors.getNotificationsByType(store);
  const latestNotifications =
    notificationSelectors.getLatestNotifications(store);
  ...
  const _getToastDuration = useCallback((priority: string) => {
    switch (priority) {
      case "low":
        return 3000;
      case "medium":
  ...
  const _getToastAction = useCallback(
    (notification: any, markAsRead: (id: string) => void) => {
  ...
  const notificationStats = useMemo(() => {
    ...
    (
      ...
      stats.byType[type] = notifications.filter(
        (n: any) => n.type === type,
      ).length;
    });
    ...
    (["low", "medium", "high", "urgent"] as string[]).forEach((priority) => {
      stats.byPriority[priority] = notifications.filter(
...
export const useRealTimeNotifications = () => {
  const { isConnected, connect, disconnect, reconnect, setConnectionStatus } =
    useNotifications();
...
export const useNotificationPolling = (intervalMs = 30000) => {
  const { fetchNotifications, fetchUnreadCount, unreadCount } =
    useNotifications();
  const _prevUnreadCountRef = useRef(unreadCount);
